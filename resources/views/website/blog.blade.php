                            ...
                            <div class="col-lg-4 col-md-6 col-12" data-aos="fade-up" data-aos-duration="700">
                                <div class="article-card bg-transparent p-0 shadow-none">
                                    <a class="article-card-img-wrapper" href="{{ route('blog-show', $blog['slug'] ?? '--') }}">
                                        <img src="{{$blog->image->name}}" alt="img"
                                            class="article-card-img rounded" width="500" height="230">
                                        @php
                                            $category = App\Models\Category::where('id',$blog->category_id)->first();
                                        @endphp
                                        <span class="article-tag article-tag-absolute rounded">{{$category->name}}</span>
                                    </a>
                                    {{-- {{dd($blogs)}} --}}
                                    <p class="article-card-published text_12 d-flex align-items-center">
                                        <span class="article-date d-flex align-items-center">
                                            <span class="icon-publish">
                                                <svg width="17" height="18" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <!-- Image -->
                                                    <image href="{{ asset($blog->image->name) }}" width="17" height="18" />
                                            ...
                                            <span class="ms-2">{{ \Carbon\Carbon::parse($blog->created_at)->format('d F, Y') }}</span>
                                        </span>
                                        <span class="article-author d-flex align-items-center ms-4">
                                            ...
                                            <span class="ms-2">{{$blog->author_name}}</span>
                                        </span>
                                    </p>
                        ...
                        <div class="pagination justify-content-center mt-100">
                            ...
                            <nav>
                                <ul class="pagination m-0 d-flex align-items-center">
                                    ...
                                    @else
                                        <li class="item">
                                            <a class="link" href="{{ $blogs->previousPageUrl() }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-left">
                                                    <polyline points="15 18 9 12 15 6"></polyline>
                                                </svg>
                                            </a>
                                    ...
                                    @for ($i = 1; $i <= $blogs->lastPage(); $i++)
                                        <li class="item @if ($i === $blogs->currentPage()) active @endif">
                                            <a class="link" href="{{ $blogs->url($i) }}">{{ $i }}</a>
                                        </li>
                                    @endfor
                                    ...
                                    @if ($blogs->hasMorePages())
                                        <li class="item">
                                            <a class="link" href="{{ $blogs->nextPageUrl() }}">
