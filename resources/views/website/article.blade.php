...
@section('content')
                        ...
                        <div class="col-lg-9 col-md-12 col-12">
                            <div class="article-rte">
                                <div class="article-img">
                                    {{-- {{dd($blogs->image->name)}} --}}
                                    <img src="{{asset($blogs->image->name ?? '')}}" alt="img">
                                </div>
                                {{-- {{dd($blogs)}} --}}
                                <div class="article-meta">
                                    <h2 class="article-title">{{$blogs->title ?? '--'}}</h2>
                                    <div class="article-card-published text_14 d-flex align-items-center">
                                        <span class="article-author d-flex align-items-center">
                                            ...
                                            <span class="ms-2">{{$blogs->created_by ?? '--'}}</span>
                                        </span>
                                        ...
                                        <a href="#" class="article-date d-flex align-items-center">
                                            ...
                                            <span class="ms-2">@if(isset($blogs->comments)) {{$blogs->comments->count()}} Comments @else No comments @endif</span>
                                        </a>
                                        ...
                                        @php
                                        if(isset($blogs))
                                        $category = App\Models\Category::where('id',$blogs->category_id)->first();
                                        @endphp
                                ...
                                <div class="article-content">
                                    <p>{!! nl2br(e($blogs->description ?? '--')) !!}</p>

                                    <figure class="blockquote">
                                        <blockquote>
                                            {{$blogs->qoute ?? '--'}}
                                        </blockquote>
                                        <figcaption>- {{$blogs->qoute_author_name ?? '--'}}</figcaption>
                                    </figure>
                                ...
                                <div class="comments-section mt-100 home-section overflow-hidden">
                                    <div class="section-header">
                                        <h2 class="section-heading">Comments - {{$blogs->comments->count()}}</h2>
                                    </div>
                                  
                                    @include('dashboard.WebsiteManagement.Comments.singleComment', ['comments' => $blogs->comments, 'blog_id' => $blogs->id])
                                    <div class="comment-form-area">
                                        <div class="form-header">
                                            <h4 class="form-title">Leave A Comment</h4>
                                            <p class="form-subtitle">All fields marked with an asterisk (*) are required</p>
                                        </div>
                                        <form class="comment-form" id="addComment" name="addComment" method="POST"
                                        enctype="multipart/form-data"
                                        action="{{ route('store-comment') }}">
                                        @csrf
                                            <div class="name-email-field d-flex justify-content-between">
                                                <div class="field-item name-field">
                                                    ...
                                                    <input type="hidden" name="blog_id" id="blog_id" @if(isset($blogs['id'])) value={{$blogs['id']}} @endif>
                                            ...
                                            <div class="field-item textarea-field">
                                                ...
                                                <textarea cols="20" rows="6" name="comment" placeholder="Write your comment here........"></textarea>
 ...
 @endsection
 <!-- Hidden Reply Box Template -->

 {{-- <div id="reply-box-template" style="display: none;">
    <form class="comment-form" id="addReply" name="addReply" method="POST"
          enctype="multipart/form-data"
          action="{{ route('store-comment') }}">
          @csrf
        <input type="hidden" name="blog_id" id="blog_id" @if(isset($blogs['id'])) value={{$blogs['id']}} @endif>
        ...
        <input type="text" name="username" placeholder="Your Username" class="form-control my-2">
        <input type="email" name="email" placeholder="Your Email" class="form-control my-2">
        <textarea placeholder="Add your reply..." name="reply" class="form-control my-2"></textarea>
...
<script>
    $(document).ready(function() {
        $('.btn-reply').on('click', function() {
            const $commentsMainContent = $(this).closest('.comments-main-content');
            console.log($commentsMainContent);
            $commentsMainContent.find('.form-control').closest('div').remove();
            const replyBox = $('#reply-box-template').clone().removeAttr('id').show();
            replyBox.find('#parent_id').val($(this).data('comment-id'));
            $commentsMainContent.append(replyBox);
        });
        $('.btn-reply1').on('click', function() {
            const $commentsMainContent = $(this).closest('.comments-main');
            console.log($commentsMainContent);
            $commentsMainContent.find('.form-control').closest('div').remove();
            const replyBox = $('#reply-box-template').clone().removeAttr('id').show();
            replyBox.find('#parent_id').val($(this).data('comment-id'));
...
<div id="reply-box-template" style="display: none;">
    <form class="comment-form" id="addReply" name="addReply" method="POST"
        ...
        <input type="text" name="name" placeholder="Your Username" class="form-control my-2 abc" required>
        <input type="email" name="email" placeholder="Your Email" class="form-control my-2 abc" required>
        <textarea placeholder="Add your reply..." name="comment" class="form-control my-2 abc" required></textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        function toggleSubmitButton(form) {
            const usernameValue = form.querySelector('input[name="name"]').value.trim();
            const emailValue = form.querySelector('input[name="email"]').value.trim();
            const replyValue = form.querySelector('textarea[name="comment"]').value.trim();
            
            const submitButton = form.querySelector('.submit-btn');
            if (usernameValue !== '' && emailValue !== '' && replyValue !== '') {
        ...
        document.querySelectorAll('.btn-reply').forEach(function(button) {
            button.addEventListener('click', function() {
                const commentsMainContent = this.closest('.comments-main-content');
                commentsMainContent.querySelectorAll('.form-control').forEach(function(control) {
                    control.closest('div').remove();
                });
                const replyBox = document.getElementById('reply-box-template').cloneNode(true);
                replyBox.removeAttribute('id');
                replyBox.style.display = 'block';
                replyBox.querySelector('#parent_id').value = this.dataset.commentId;
                commentsMainContent.appendChild(replyBox);

                replyBox.querySelectorAll('.abc').forEach(function(field) {
                    field.addEventListener('input', function() {
                        toggleSubmitButton(replyBox);
                    });
                });
            });
        });
